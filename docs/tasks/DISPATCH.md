...
> 所有角色的委派统一在此登记。扫描与唤醒机制采用事件驱动模式（OS cron 脚本零 token 扫描，有任务时写 trigger 文件按需唤醒 LLM），详见 framework 事件驱动唤醒模式文档。
...
**扫描与唤醒**：OS cron 脚本零 token 扫描各行状态，检测到交付（📬）时写 trigger 文件，AI 工具监听 trigger 文件后唤醒 LLM 触发验收（交付清单通过 + 状态清单整体完成状态 ✅）→ 在交付清单写入验收结果，委派清单改 ✅。扫描机制详见 framework 事件驱动唤醒模式。  
